...
/*------------------------ Public wrappers. --------------------------------*/
/* Structure which stores the node which stores information about each memory location */
struct mem_node {
  void *mem_address;
  int count;
...
/* Head of the list. */
struct mem_node * list_head;
struct mem_node *__create_node (void * address);
void __insert_node(void *address);
struct mem_node *__find_node(void *address);
void __update_value (void *address);
void mem_access_stat();

/* Function for inserting an element in the list */ 
void __insert_node(void *address){
  struct mem_node * mem_node_elem = __create_node (address);
  if (list_head == NULL)
    list_head = mem_node_elem;
  else {
    mem_node_elem->next = list_head;
    list_head = mem_node_elem; 
  } 
}

/* Function which creates a node */
struct mem_node *__create_node (void * address){
  struct mem_node * mem_node_elem = (struct mem_node *) __libc_malloc (sizeof (struct mem_node));
  mem_node_elem->mem_address = address;
  mem_node_elem->count = 0;
  mem_node_elem->next = NULL;
  return mem_node_elem;
}

/* Function which finds a node*/ 
struct mem_node *__find_node(void *address){
  struct mem_node *curr_elem = list_head;
  while (curr_elem != NULL && curr_elem->mem_address != address){
    curr_elem = curr_elem->next;
  }
  if (curr_elem->mem_address == address)
    return curr_elem;
  return NULL;
}

/* Function which updates the value of a node */
void __update_value (void *address){
  struct mem_node *mem_node_elem = __find_node (address);
  if (mem_node_elem != NULL){
    void *header = (void *)((int *)address - 1);
    mem_node_elem->count = *((int *)(header));
  }
}
...
void mem_access_stat(){
  ...
  while (curr_elem != NULL){
    printf("Address = 0x%lx, Access Count = %d\n", (long unsigned int)curr_elem->mem_address,curr_elem->count);
    curr_elem = curr_elem->next;
  }
}
...
void *hmalloc (size_t bytes){
  ...
  void *victim = __libc_malloc (bytes);
  victim = (void *)((int *)victim + 1);
  return victim;
 }

/**/
void *hmalloc_stat (size_t bytes){
  bytes = bytes + sizeof (int); // adding the header as an integer
  void *victim = __libc_malloc (bytes);
  victim = (void *)((int *)victim + 1);
  __insert_node (victim);
  return victim;
}

/* Implementation of the free function, which frees the header */
void hfree (void *memory_address){
  void *header = (void *)((int *)memory_address -1);
  __libc_free (header);
}

void hfree_stat (void *memory_address){
  __update_value (memory_address);
  void *header = (void *)((int *)memory_address -1);
  __libc_free (header);
}

/* Implementation of the memory access method */
void mem_access(void *memory_location, int count){
   int *x = ((int *)memory_location-1);
   (*x) += count;
}

/* Implementation of the method which returns the amount of memory accessed for each memory location */
int mem_access_count (void *object_address){
