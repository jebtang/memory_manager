...
\paragraph{}
This work presents the design of Tracer. Tracer provides a C pre-processing tool called the {\emph{interceptor}} and a set of APIs (bundled with the standard GNU C library) for monitoring and providing the overall memory access footprint at an object level granularity to C application developers. The interceptor uses preprocessing through standard code parsing techniques and injects {\emph{"memory access"}} calls into the C program, thus making the design transparent to the application developer and the system easily integrated into standard C applications. The monitoring library is implemented as a part of the standard GNU C library, and provides a simple interface for the developer to obtain relevant memory statistics.  The solution does not depend on specific hardware and is therefore independent of the platform used. Our implementation on binary tree micro benchmarks report an overhead between roughly $20\%$ and $40\%$ for create and read workloads. We therefore believe that Tracer is a useful software solution which could augment application development by providing more efficient memory monitoring techniques.


\paragraph{}
The rest of this paper is organized as follows: Section \ref{sec:motivation} outlines the motivation underlying our work.  Section \ref{sec:design} describes the design of Tracer. Section \ref{sec:eval} describes the microbenchmark study and discussion of results. Section \ref{sec:futurework} provides insight into useful ideas which could improve the design of Tracer. Section \ref{sec:related} discusses some of the previous pieces of work related to Tracer's design. Section \ref{sec:conclusion} provides a summary of our work.
